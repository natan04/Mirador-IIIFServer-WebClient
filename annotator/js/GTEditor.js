...
function GTEditor(docsManager, scheme, editorDiv, page) {
    ...
    this.initPanel = function () {
        ...
        _controlPanel.append(
            ...
            newDiv().addClass('button').attr('title', 'help')
                .addClass('helpIcon')
    ...
    this.mainKeyHandler = function (e) {
        ...
        if (key == 219) _documentDiv.addClass('smallTranscription');
        if (key == 221) _documentDiv.removeClass('smallTranscription');
    }
	...
	if (_selected!=null)
	{
		selectedElementData = _selected.data('element');
	        if ((Math.pow(selectedElementData.Y - bb.Y, 2) + Math.pow(selectedElementData.X - bb.X, 2) + Math.pow(bb.Height - selectedElementData.Height, 1) + Math.pow(bb.Width - selectedElementData.Width,2) ) < 10)
	        {
	            alert('duplicated element has been removed');
	            return
	        }
	}
        bb.OriginX = x;
